...
def getBookList(author_url,author_name,filetypes=['PDF','MP3']):
    ...
    print('Getting list of books by {} @ {} ...'.format(author_name,author_url))
    ...
    for row in soup.find(class_='plain-list').find_all('a',href=True):        
        ...
        print(book_name)
        site_url = '{}{}'.format(base_url,row['href'])
        ...
        soup_book = BeautifulSoup(source_book,'lxml') 
        
        temp_dict = {'book_name':book_name,'site_url':site_url}
        
        soup_urls = soup_book.find(class_='book-box-formats').find_all('a',href=True,class_=False)
        for filetype in filetypes:
            for elem in soup_urls:            
                if elem.string == filetype:
                    temp_dict['{}_url'.format(filetype)] = '{}{}'.format(base_url,elem['href'])
...
def getBookPart(df,author_name,wd,file_type='PDF',unzip=False):    
    ...
    wd_author = '{}{}/'.format(wd,author_name)
    ...
    if len(df['{}_url'.format(file_type)].dropna())>0:
        ...
        if not os.path.exists('{}/{}/'.format(wd_author,file_type)):
            os.makedirs('{}/{}/'.format(wd_author,file_type))
        ...
        for file_url in df['{}_url'.format(file_type)].dropna():
            try:
                ...
                open('{}{}/{}'.format(wd_author,file_type,rep_name), 'wb').write(r.content)
...
def scraperWL(author_url,author_name,wd,filetypes=['PDF','MP3'],unzip=False):
    df = getBookList(site_url,author,wd)
